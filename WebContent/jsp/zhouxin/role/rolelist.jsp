...
<div class="site_content">
	<div class="content_query">
		<form method="post" id="queryFm">
			...
			<input type="hidden" id="selRoleId" name="selRoleId">
			角色编码：<input type="text" name="q_roleCode" value="${queryMap.roleCode}" maxlength="10">
			角色名称：<input type="text" name="q_roleName" value="${queryMap.roleName}" maxlength="10">
			<input class="btn" type="button" value="查询" onclick="queryRole()">
		</form>
	</div>
	...
	<div class="content_list">
		...
		<c:forEach var="role" items="${roleList}">
			<tr>
				...
				<td>${role.lastUpdateBy}</td>
				<td>${role.lastUpdateTime}</td>
				<td>
					<a href="javascript:void(0);">查看</a>
			<c:forEach var="resource" items="${sessionScope.CurrentUserResources}">
				<c:if test="${resource.uri eq '/role.do?method=delete'}">
					<a href="javascript:void(0);" onclick="delRole(${role.roleId})">删除</a>
				</c:if>
				<c:if test="${resource.uri eq '/role.do?method=toAssignResource'}">
					<a href="javascript:void(0);" onclick="assignResource(${role.roleId})">分配资源</a>
				</c:if>
			</c:forEach>
				</td>
			</tr>
...
<script type="text/javascript">
	...
	function queryRole(){
		...
		qFm.action = "${pageContext.request.contextPath}/role.do?method=list";
		qFm.submit();
	}
	function delRole(roleId){
		if(confirm("确定删除该角色？")){
			...
			qFm.action = "${pageContext.request.contextPath}/role.do?method=delete";
			qFm.submit();
		}
	}
	function assignResource(roleId){
		var qFm = document.getElementById("queryFm");
		document.getElementById("selRoleId").value=roleId;
		qFm.action = "${pageContext.request.contextPath}/role.do?method=toAssignResource";
		qFm.submit();
	}
